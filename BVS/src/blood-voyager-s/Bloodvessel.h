...
#include "../../../Terahertz_channel_C_code/terahertz.h"
#include "../../../Ultrasonic_channel_C_code/ultrasound-channel.h"
...
private:
  bool m_start;
  vector<Ptr<Bloodstream>> m_bloodstreams; // list of bloodstreams in the 
  ...
  Ptr<PrintNanobots> m_printer;
  Ptr<Terahertz> th_channel;
  Ptr<Ultrasound> us_channel;

  ///Information for nanobot message handling
  list<Ptr<Nanobot>> nanobotsCurrentlyInThisVessel;

  std::vector<Ptr<Bloodvessel>> m_successor_vessels;

  /// Stream settings
  int m_numberOfStreams; // number of streams, maximum value is 21
  ...
  Vector CalcPosition (Vector nbv, double distance);
  ...
  // caculate streamdirection
  int CalcStreamDir(Ptr<Bloodstream> stream);

  //check whether round is completed
  void CheckRoundCompleted(Ptr<Nanobot> nb);
  ...
  */
  ...
  void DoChangeStreamIfPossible (Ptr<Bloodstream> curStream, int desStream);

  bool SuccessorVesselIsBloodDeliverVessel();

  bool SuccessorVesselIsOrgan();

  /// Transposes Nanobots from one bloodvessel to another.
  void TransposeNanobots (list<Ptr<Nanobot>> reachedEnd);

  /// Moves one Nanobot to the next bloodvessel
  void transposeNanobot (Ptr<Nanobot> botToTranspose, Ptr<Bloodvessel> nextBloodvessel);
  /**
   * /return the remaining movement exceeding the vessel nanobot is currently in
   */
  void moveNanobot (Ptr<Nanobot> nb, Ptr<Bloodstream> stream, int randVelocityOffset, bool direction);

  /**
   * @brief returns true if vesselExceedingRemainder is smaller than bloodvesselLength or if it exceeds vessel on z-plane
   * 
   * @param nb 
   * @return true 
   * @return false 
   */
  bool ExceedsVessel(Ptr<Nanobot> nb, bool fromTranspose);

  bool ExceedsVesselAgain(Ptr<Nanobot> nb);
...
public:
  ...
  */
  Bloodvessel (int id, BloodvesselType type, Vector start, Vector end, double width, Ptr<PrintNanobots> printer);
  ...
  */
  ...
  list<Ptr<Nanobot>> GetNanobotList();
  ...
  */
  ...
  void SetSuccessorVessel(Ptr<Bloodvessel> successor_vessel);
