...
PrintNanobots::PrintNanobot (Ptr<Nanobot> n, int vesselID)
{
  ...
  int BvID = vesselID;
  int st = n->GetStream ();
  bool rec = n->GetReceivedPacket();
  int hops = n->GetHopCount ();
  int round_number= n->GetVisitedGateway();
  output << id << "," << x*100 << "," << y*100 << "," << z*100 << "," << m_start << "," << BvID << "," << st << "," << rec << "," << hops << "," << round_number
         << "\n";
}
